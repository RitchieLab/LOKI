...
class Source_disgenet(loki_source.Source):
    ...
    def update(self, options):
        ...
        namespaceID = self.addNamespaces(
            [("disgenet_id", 0), ("entrez_gid", 0), ("disease", 0)]
        )
        ...
        cur.execute(comm)
        diseaseClass = {
            diseaseclass[0]: diseaseclass[1].strip() for diseaseclass in cur.fetchall()
        }
        ...
        listAID = self.addTypedGroups(
            typeID["disease"],
            (
                (
                    (
                        subtypeID["-"]
                        if diseases[diseaseID][1] is None
                        else listSubtype[diseaseClass[diseases[diseaseID][1]]]
                    ),
                    diseases[diseaseID][0],
                    None,
                )
                for diseaseID in listGroup
            ),
        )
        ...
        self.addGroupNamespacedNames(
            namespaceID["disgenet_id"],
            ((groupAID[diseaseID], diseaseID) for diseaseID in listGroup),
        )
        self.addGroupNamespacedNames(
            namespaceID["disease"],
            ((groupAID[diseaseID], diseases[diseaseID][0]) for diseaseID in listGroup),
        )
        diseases = None
        diseaseClass = None
        self.log(" OK\n")
        ...
        self.addGroupMemberTypedNamespacedNames(
            typeID["gene"], namespaceID["entrez_gid"], diseaseGene
        )
        diseaseGene = None
        self.log(" OK\n")
