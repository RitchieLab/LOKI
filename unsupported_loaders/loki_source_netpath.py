...
class Source_netpath(loki_source.Source):
    ...
    def update(self, options):
        ...
        with zipfile.ZipFile("NetPath_GeneReg_TSV1.zip", "r") as pathZip:
            ...
            for info in pathZip.infolist():
                ...
                if info.filename == "NetPath_Gene_regulation_all.txt":
                    ...
                    if not header.decode().startswith(
                        "Gene regulation id	Pathway name	Pathway ID	Gene name	Entrez gene ID"
                    ):  # 	Regulation	Experiment	PubMed ID
                        self.log(" ERROR\n")
                        self.log(
        ...
        self.log(
            " OK: %d pathways, %d associations (%d identifiers)\n"
        ...
        listGID = self.addTypedGroups(
            typeID["pathway"],
            ((subtypeID["-"], pathName[pathID], None) for pathID in listPath),
        )
        ...
        self.addGroupNamespacedNames(
            namespaceID["netpath_id"],
            ((pathGID[pathID], pathID) for pathID in listPath),
        )
        self.addGroupNamespacedNames(
            namespaceID["pathway"],
        ...
        for ns in nsAssoc:
            self.addGroupMemberTypedNamespacedNames(
                typeID["gene"],
                namespaceID[ns],
                ((pathGID[assoc[0]], assoc[1], assoc[2]) for assoc in nsAssoc[ns]),
            )
        self.log(" OK\n")
